...
> Status: Phase 0, Phase 1, and Phase 2 completed. Current focus: Phase 3 community platform, notifications, and hosting.
...
### Phase 3 — Community Platform ✅ FEATURE-COMPLETE (2026-08-01)
...
- [x] Notifications: alerts for "new publication/researcher intersecting with your work" (`scripts/generate_notifications.py`, three signals: own works, topically adjacent new works, strong collaboration candidates). Idempotent via `notifications.dedupe_key`, so it can run on a schedule.
- [ ] Hosting: **prepared, not deployed** — `DEPLOYMENT.md` documents both paths, `scripts/preflight.py` gates the deploy, `Dockerfile`/`fly.toml`/`.env.example` are ready. Awaiting a decision between free static (Pages) and full backend (Fly.io); domain not yet purchased.
...
**Decide the hosting shape**, then execute it (see `DEPLOYMENT.md`):

- *Static, free* — GitHub Pages already publishes `site/` after CI passes. No login/admin/profile.
- *Full app* — Fly.io with a mounted volume for `data/syriac.db`. Requires `SECRET_KEY`, a seeded volume, and registering the admin account immediately (the first account to register becomes admin).

In parallel, two curation tasks are open and do not block deployment:

1. Work through the 920 pending rows in `duplicate_candidates` via the admin UI (the 5 same-DOI pairs deliberately left for human judgement are the highest-value ones — editions and reprints).
2. Consider adding abstracts to the embedding input; titles alone remain the main quality ceiling for clustering and collaboration candidates.
...
- 2026-07-31: Follow-up verification completed. Re-ran `scripts/export_json.py`; `site/data.json` now matches the database (6,554 works, 2,779 authors, 8,127 citations, 31,180 similarity edges, 41 substantial clusters, 300 collaboration candidates). `scripts/check_data.py` reports 0 errors and 1 warning (23 duplicate DOI groups). Full unittest suite passes (`10/10`). API smoke test passed on a temporary database copy: `/healthz`, admin protection for `/api/status`, first-user admin bootstrap, login, and admin work create/update/delete. README and PLAN refreshed. Remaining decision: free static sharing via GitHub Pages vs full authenticated backend hosting.
- 2026-08-01: CI hardening + API test coverage. GitHub Actions previously only published `site/` on every push, with no test gate. Added `.github/workflows/ci.yml` (compileall + unittest + `check_data.py` + `preflight.py --target pages`) and rewired `pages.yml` to trigger on `workflow_run` after CI succeeds, so a failing test or a stale export can no longer reach production. Added `tests/test_api.py` (19 tests) covering the previously untested FastAPI layer: admin bootstrap, admin-only protection on every mutating endpoint, work lifecycle, manuscript details, merge relation remapping, and the contribution moderation queue. **Bug found by the new merge test:** `merge_works` referenced a non-existent `citations.work_id` column (the schema uses `citing_work_id`), so *every* duplicate merge through the admin UI failed — fixed, and the merge now also clears self-citations created by remapping.
- 2026-08-01: Phase 3 notifications implemented (`scripts/generate_notifications.py`), closing the last open Phase 3 feature. Three signals for users who claimed an author profile: (1) works crediting them, (2) works in the same Leiden cluster connected to one of their works by a similarity edge — the "intersects with your work" alert from the vision statement, (3) `collaboration_candidates` above 0.55 (deliberately stricter than the 0.35 candidate-table bar, since PLAN.md documents that weak pairs still carry false positives). Idempotency comes from a new `notifications.dedupe_key` column with a partial UNIQUE index, so the script is cron-safe; collaboration keys are order-independent so a reversed pair cannot double-notify. Capped at 10 rows per user per signal. Endpoints gained `unread_only`, `/unread-count` and `/read-all`, and per-notification writes are now owner-scoped in a single statement. Verified against real data: the Sebastian P. Brock profile produced 30 notifications, and a second run produced 0.
- 2026-08-01: Same-DOI duplicate resolution (`scripts/resolve_duplicates.py`). A shared DOI turned out **not** to be sufficient proof of duplication in this corpus — publishers reuse DOIs across editions ("The Bible in the Syriac Tradition" 2002 vs "(Third Edition)" 2021) and across an 1894 original and its 2012 reprint. The script therefore auto-merges only prefix/subtitle-only title variants with no edition/volume marker and years within 3, and queues everything else for a curator. Applied: **18 groups merged, 5 queued**; live duplicate-DOI groups dropped 23 → 5. Survivor selection prefers a clean title over mojibake (an earlier revision would have enshrined `Erica<?>C.<?>D. Hunter` on the surviving node), then citation count, then title length, and the merge adopts the richer title/venue. Also fixed `check_data.py`, whose duplicate-DOI query counted soft-deleted rows — meaning every completed merge would have warned forever. Re-ran the analysis chain: 6,536 live works, 8,118 citations, 31,013 similarity edges, 37 clusters, 0 integrity errors.
- 2026-08-01: Deployment prepared but **deliberately not executed** (user decision: prepare only). Added `DEPLOYMENT.md` comparing static Pages vs full Fly.io backend, `.env.example`, and `scripts/preflight.py` — a runtime-readiness gate distinct from `check_data.py`: it checks `SECRET_KEY`, wildcard CORS, migration state, admin-account presence, and that `site/data.json` is newer than `data/syriac.db` (shipping a stale export would serve a graph that silently disagrees with the database). Dockerfile now honours `${PORT}` instead of hardcoding 8080 and has a `/healthz` HEALTHCHECK. Site copy corrected: the collaboration panel still claimed "based on title text only" long after the pipeline moved to multilingual embeddings. Test suite now 63 tests, all passing.
